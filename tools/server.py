...
def run_backtest(
    ...
    """Run a backtest replay on historical data. The agent walks through each bar sequentially with NO access to future data.

    When to use: Testing a strategy against historical data before risking real capital. The simulation broker fills all market orders at the current bar's close price. Use load_price_cache first to ensure data is available.

    Sample input: run_backtest("TSLA", "2026-01-01", "2026-05-21", "1Day", 100000.0)

    Expected output:
    {"symbol": "TSLA", "total_bars": 98, "initial_capital": 100000, "final_equity": 105230.50,
     "total_pnl": 5230.50, "total_pnl_pct": 5.23, "open_positions": 0,
     "trades": [{"bar": 5, "action": "buy", ...}, ...]}

    IMPORTANT: This runs the simulation broker — the agent cannot see future data.
    Call load_price_cache first to load the historical data, then call this.
    The agent should use get_market_data/calc_technical_indicators at each bar to make decisions.
    """
    from backtest.engine import run_replay, step_through

    setup = run_replay(get_repo(), symbol, start_date, end_date, timeframe, initial_capital)
    if "error" in setup:
    ...
    # Step through all bars — no agent logic here (agent calls tools separately)
    # This tool sets up the replay and returns the bar timestamps so the agent
    # can iterate and make decisions using the normal tools
    result = step_through(sim, bars)

    # Store the sim broker globally so subsequent tool calls use it
    global _broker
    _broker = sim
    ...
    result["mode"] = "replay_complete"
    result["note"] = "Replay finished. Use query_transaction_ledger to see all trades made during replay."
...
def setup_backtest(
    ...
    """Set up a backtest environment WITHOUT stepping through bars. Swaps the broker to simulation mode so subsequent tool calls (get_market_data, place_order, etc.) use the simulation broker.

    When to use: When you want the AGENT to step through bars manually, making decisions at each one. After calling this, use advance_bar to move forward one bar at a time.

    Sample input: setup_backtest("TSLA", "2026-01-01", "2026-06-01")

    Expected output:
    {"status": "ready", "symbol": "TSLA", "total_bars": 105, "first_bar": "2026-01-02", "last_bar": "2026-05-30"}
    """
    ...
    global _broker, _backtest_bars, _backtest_index
    _broker = sim
    _backtest_bars = bars
    _backtest_index = 0
    ...
    try:
        sim.set_time(datetime.fromisoformat(first_ts))
    except (ValueError, TypeError):
        sim.set_time(datetime.strptime(str(first_ts), "%Y-%m-%d"))
...
@mcp.tool()
def advance_bar() -> str:
    """Advance the backtest simulation by one bar. Returns the new bar's data.

    When to use: During a backtest, call this to move to the next time period. Then use get_market_data, calc_technical_indicators, etc. to analyze and decide whether to trade.

    Sample input: advance_bar()

    Expected output:
    {"bar_index": 5, "timestamp": "2026-01-08", "open": 415.2, "high": 420.1,
     "low": 413.8, "close": 418.5, "volume": 52000000, "remaining": 93,
     "account": {"equity": 100500, "cash": 80000, "positions": 1}}

    Returns {"done": true} when all bars have been processed.
    """
    global _backtest_bars, _backtest_index, _broker

    if not hasattr(advance_bar, '__wrapped__') and '_backtest_bars' not in globals():
        return json.dumps({"error": "No backtest active. Call setup_backtest first."})

    bars = _backtest_bars
    idx = _backtest_index

    if idx >= len(bars):
        # Backtest complete — return final summary
        account = _broker.get_account()
        pnl = account["equity"] - _broker.initial_capital
        return json.dumps({
            "done": True,
            "final_equity": round(account["equity"], 2),
            "total_pnl": round(pnl, 2),
            "total_pnl_pct": round(pnl / _broker.initial_capital * 100, 2),
            "open_positions": len(_broker.positions),
        })

    bar = bars[idx]
    _backtest_index = idx + 1

    # Advance simulation clock
    ts = bar["timestamp"]
    try:
        _broker.set_time(datetime.fromisoformat(ts))
    except (ValueError, TypeError):
        _broker.set_time(datetime.strptime(str(ts), "%Y-%m-%d"))
    ...
    return json.dumps({
        ...
        "account": {
            ...
            "positions": len(_broker.positions),
        },
    }, default=str)


# Backtest state globals
_backtest_bars: list[dict] = []
_backtest_index: int = 0
